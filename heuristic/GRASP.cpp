...
void grasp(string filename, int n, int p, int r);
...
void grasp(string filename, int n, int p, int r) {
	...
	// cout << "MAX_ITERATION: " << MAX_ITERATIONS << endl;
	for (int i = 0; i < MAX_ITERATIONS; i++) {
		...
		try {
			...
			if (instanceType != 2) {
				...
				string _filename = "grasp_" + to_string(getpid()) + "_" + to_string(time);
				LKHParser parser(LKH_PATH, LK_FILES_PATH, gvertices, d, _filename, string("TSP"));
			...
			double currSol = model.get(GRB_DoubleAttr_ObjVal);
			double currLower = model.get(GRB_DoubleAttr_ObjBound);
			// cout << currSol << endl;
			if (currSol < bestSol) {
				bestSol = currSol;
				bestLowerBound = currLower;
			}
			// SOLUTION
